...
@section('title', 'Ordenes Combustible')
@section('content_header')

<h1>Ordenes Combustible</h1>
@stop
@section('content')
...
<div class="card">
    ...
    <div class="card-body">
        @php
        $heads = ['ID', 'Datos Conductor','Datos Vehículo','Solicitante','Gasolinera','Galones de combustible','Fecha Hora','Kilometraje Actual','Fecha solicitud', ['label' => 'Acciones', 'no-export' => true, 'width' => 15]];
        ...
        <x-adminlte-datatable id="table1" :heads="$heads" :config="$config" class="table table-striped table-bordered">
            @foreach ($ordenCombustibles as $orden)
            <tr>
                <td>{{ $orden->id_orden }}</td>
                <td>{{ $orden->nombres.' '.$orden->apellidos }}</td>
                <td>{{ $orden->marca.' '.$orden->placa }}</td>
                <td>{{ $orden->nombres.' '.$orden->apellidos }}</td>
                <td>{{ $orden->nombre_gasolinera }}</td>
                <td>{{ $orden->cantGalonesGasolina }}</td>
                <td>{{ $orden->fecha }}</td>
                <td>{{ $orden->kilometraje_actual }}</td>
                <td>{{ $orden->fecha }}</td>
                <td>
                    <form action="{{ route('OrdenCombustibles.destroy', $orden->id_orden) }}" method="POST">
                        <a class="btn btn-sm btn-success" href="{{ route('OrdenCombustibles.edit', $policia->id) }}"><i class="fa fa-fw fa-edit"></i></a>
                        @csrf
                        @method('DELETE')
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('exportToExcel').addEventListener('click', function() {
            ...
            if (allData.length > 0) {
                ...
                XLSX.writeFile(wb, 'OrdenesCombustible.xlsx');
            } else {
        ...
        document.getElementById('exportToPDF').addEventListener('click', function() {
            ...
            html2canvas(table).then(canvas => {
                ...
                const title = 'Reporte Ordenes Combustible';
                ...
                pdf.save('ordenesCombustible.pdf');
            });
        });
    });
</script>
